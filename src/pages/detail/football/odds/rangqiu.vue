<template>
    <div v-if="rangqiu">
        ...
        <no-data v-if="!rangqiu.odds || !rangqiu.odds.length"></no-data>


        <div class="pl-box-bd" v-if="rangqiu.odds && rangqiu.odds.length">
            <table cellspacing="0" cellpadding="0" border="0" class="pl-table" width="100%">
                <colgroup>
                    <col width="19%">
                    <col width="7%">
                ...
                <tr :class="{'yebg': info.cid === $route.query.cid}" v-for="info in rangqiu.odds"
                    ...
                    <td  class="colorc">
                        <p class="pl-num"><span :class="{'red': info.first.w == 1, 'green': info.first.w == -1}">{{parseFloat(info.first.win).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.first.d == 1, 'green': info.first.d == -1}">{{parseFloat(info.first.draw).toFixed(2)|nullFormat}}</span>/<span  :class="{'red': info.first.l == 1, 'green': info.first.l == -1}">{{parseFloat(info.first.lost).toFixed(2)|nullFormat}}</span></p>
                        <p class="pl-num"><span :class="{'red': info.end.w == 1, 'green': info.end.w == -1}">{{parseFloat(info.end.win).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.end.d == 1, 'green': info.end.d == -1}">{{parseFloat(info.end.draw).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.end.l == 1, 'green': info.end.l == -1}">{{parseFloat(info.end.lost).toFixed(2)|nullFormat}}</span></p>
                    </td>

                    <td>
                        <p class="pl-num"><span  :class="{'red': info.first.wkl == 1, 'green': info.first.wkl == -1}">{{parseFloat(info.first.winkl).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.first.dkl == 1, 'green': info.first.dkl == -1}">{{parseFloat(info.first.drawkl).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.first.lkl == 1, 'green': info.first.lkl == -1}">{{parseFloat(info.first.lostkl).toFixed(2)|nullFormat}}</span></p>
                        <p class="pl-num"><span  :class="{'red': info.end.wkl == 1, 'green': info.end.wkl == -1}">{{parseFloat(info.end.winkl).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.end.dkl == 1, 'green': info.end.dkl == -1}">{{parseFloat(info.end.drawkl).toFixed(2)|nullFormat}}</span>/<span :class="{'red': info.end.lkl == 1, 'green': info.end.lkl == -1}">{{parseFloat(info.end.lostkl).toFixed(2)|nullFormat}}</span></p>
        ...
        <div class="sk-btips" v-if="rangqiu && rangqiu.odds && rangqiu.odds.length">
            共{{rangqiu.all_num}}家公司为你提供数据，其中主流公司{{rangqiu.main_num}}家
            <br>
...
//            await store.dispatch(aTypes.getOddsRq, params.fid)
        },
        components: {
            noData
        },
        ...
        methods: {
            async fetchData () {
                ...
                await this.$store.dispatch(aTypes.getOddsRq, this.$route.params.fid)
        ...
        computed: {
            ...
            rangqiu () {
                return this.$store.state.zqdetail.odds.rangqiu
            }
        },
        filters: {
            nullFormat: (value) => {
